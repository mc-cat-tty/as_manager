...
namespace as::ebs_supervisor{
    ...
    auto asms_signal= Signal<bool>(hal::read_asms_status,&updater, Parameters::getInstance().asmsAlpha);
    ...
    auto sdc_signal = Signal<hal::SdcState>(hal::read_sdc,&updater, Parameters::getInstance().sdcAlpha);
    auto breake_pressure_front_signal = Signal<float>(hal::read_brake_pressure_front,&updater,Parameters::getInstance().brakePressureFrontAlpha);
    auto breake_pressure_rear_signal = Signal<float>(hal::read_brake_pressure_rear,&updater,Parameters::getInstance().brakePressureRearAlpha);
    ...
    auto rpm_signal= Signal<float>(hal::read_rpm,&updater,Parameters::getInstance().rpmAlpha);
}
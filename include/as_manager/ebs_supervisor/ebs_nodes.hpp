...
namespace as::ebs_supervisor {
    ...
    constexpr inline auto WAIT_MISSION_ASMS_NODE=waitUntilNode(
      waitAsmsAnsMission, 
      "Mission selected and ASMS ON", "Waiting for mission and ASMS", [] {}
    );

    constexpr inline auto ASSERT_EBS_PRESSURE_NODE = assertWithTimeoutNode(
      []{
    ...
    constexpr inline auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_ALL_ACT_NODE = assertWithTimeoutNode(
      []{
    ...
    constexpr inline auto OPEN_SDC_NODE = doActionNode(open_sdc, "Open SDC");
    constexpr inline auto CLOSE_SDC_NODE = doActionNode(close_sdc, "Close SDC");

    constexpr inline auto ASSERT_SDC_OPEN_NODE = assertWithTimeoutNode(
      []{
    ...
    constexpr inline auto ASSERT_SDC_CLOSE_NODE = assertWithTimeoutNode(
      []{
        return sdc_signal.get_value_with_threhold(SDC_TRESHOLD_CLOSE);
      }, 500ms, "SDC close", "Waiting SDC closing", "SDC closing timeout");
        
    constexpr inline auto TOGGLING_WATCHDOG_NODE = doActionNode([]{Watchdog::getInstance().set_toggling();}, "Start toggling watchdog");
    constexpr inline auto STOP_TOGGLING_WATCHDOG_NODE = doActionNode([]{Watchdog::getInstance().stop_toggling();}, "Stop toggling watchdog");

    constexpr inline auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_NODE = assertWithTimeoutNode(
      []{
        std::cout<<"PBRAKE: "<<breake_pressure_rear_signal.get_value()<<" "<<breake_pressure_front_signal.get_value()<<std::endl;
    ...
    constexpr inline auto ASSERT_NO_BRAKE_PRESSURE_NODE = assertWithTimeoutNode(
      []{
        std::cout<<"PBRAKE: "<<breake_pressure_rear_signal.get_value()<<" "<<breake_pressure_front_signal.get_value()<<std::endl;
    ...
    constexpr inline auto UNBRAKE_ACT1_NODE = doActionNode(unbrake_act1, "Unbrake Act1");
    constexpr inline auto BRAKE_ACT1_NODE = doActionNode(brake_act1, "brake Act1");
    constexpr inline auto BRAKE_ACT2_NODE = doActionNode(brake_act2, "brake Act2");
    constexpr inline auto UNBRAKE_ACT2_NODE = doActionNode(unbrake_act2, "Unbrake Act2");

    constexpr inline auto WAIT_BRAKE_MOTOR_ENALBED = waitUntilNode(
      []{
    ...
    constexpr inline auto BRAKE_WITH_MAXON_MOTOR_NODE = doActionNode(brake_with_maxon, "Brake with Maxon motor");

    constexpr inline auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_WITH_MAXON_MOTOR = assertWithTimeoutNode(
      []{
    ...
    constexpr inline auto WAIT_GO_SIGNAL_WITH_CONTINUOS_MONITORING_NODE = waitUntilNode<SafetyMonitoringSwitch::ENABLE>(
      []{
    ...
    constexpr inline auto WAIT_STOP_SIGNAL_WITH_CONTINUOS_MONITORING_NODE =  waitUntilNode<SafetyMonitoringSwitch::ENABLE>(
      []{
    ...
    constexpr inline auto WAIT_TS_ACTIVE = waitUntilNode(
      []{
    ...
    constexpr inline auto PULL_CLUTCH_NODE = doActionNode(pullClutch, "Pulling clutch");
    constexpr inline auto GEAR_FIRST_NODE = doActionNode(setFirstGear, "Setting gear to first");
}
...
namespace as::ebs_supervisor {
    ...
    auto ASSERT_EBS_PRESSURE_NODE = assertWithTimeoutNode(
      ...
      Parameters::getInstance().assertEbsPressureTimeoutMs,
      "Sufficient EBS tank pressure", "Waiting for sufficient EBS pressure", "EBS pressure timeout"
    );
    ...
    auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_ALL_ACT_NODE = assertWithTimeoutNode(
      ...
      Parameters::getInstance().assertAllBrakesPressureTimeoutMs,
      "Sufficient BRAKE pressure on both actuators", "Waiting sufficient BRAKE pressure on both actuators", "BRAKE pressure on both actuators timeout"
    );
    ...
    auto ASSERT_SDC_OPEN_NODE = assertWithTimeoutNode(
      []{
        return hal::read_sdc() == hal::SdcState::OPEN;
      },
      Parameters::getInstance().assertSdcOpenTimeoutMs,
      "SDC open", "Waiting SDC opening", "SDC opening timeout"
    );

    auto ASSERT_SDC_CLOSE_NODE = assertWithTimeoutNode(
      []{
        return hal::read_sdc() == hal::SdcState::CLOSE;
      },
      Parameters::getInstance().assertSdcCloseTimeoutMs,
      "SDC close", "Waiting SDC closing", "SDC closing timeout"
    );
    ...
    auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_NODE = assertWithTimeoutNode(
      ...
      Parameters::getInstance().assertOneBrakePressureTimeoutMs,
      "Sufficient BRAKE pressure", "Waiting sufficient BRAKE pressure", "BRAKE pressure timeout"
    );
    ...
    auto ASSERT_NO_BRAKE_PRESSURE_NODE = assertWithTimeoutNode(
      ...
      Parameters::getInstance().assertNoBrakePressureTimeoutMs,
      "No brake pressure", "Waiting no brake pressure", "Brake pressure timeout"
    );
    ...
    auto ASSERT_SUFFICIENT_BRAKE_PRESSURE_WITH_MAXON_MOTOR = assertWithTimeoutNode(
      []{
      ...
      Parameters::getInstance().assertMaxonBrakePressureTimeoutMs,
      "Sufficient BRAKE pressure with MAXON", "Waiting sufficient brake pressure with MAXON", "Brake pressure MAXON timeout"
    );

    auto WAIT_GO_SIGNAL_OFF_NODE = waitUntilNode<SafetyMonitoringSwitch::ENABLE>(
      []{
        return !hal::utils::mask(hal::read_res_bit_vector(), (unsigned) hal::Res::GO);
      },
      "GO signal OFF received", "Waiting for GO signal OFF",
      [] { EbsContinousMonitoring::getInstance().continuousMonitoring(); }
    );

    auto WAIT_GO_SIGNAL_ON_NODE = waitUntilNode<SafetyMonitoringSwitch::ENABLE>(
      []{
        return hal::utils::mask(hal::read_res_bit_vector(), (unsigned) hal::Res::GO);
      },
      "GO signal ON received", "Waiting for GO signal ON",
      [] { EbsContinousMonitoring::getInstance().continuousMonitoring(); }
    );

    auto WAIT_STOP_SIGNAL = waitUntilNode<SafetyMonitoringSwitch::ENABLE>(
      hal::read_stop_message, "STOP signal received", "Waiting for STOP signal",
      [] { EbsContinousMonitoring::getInstance().continuousMonitoring(); }
    );

    auto WAIT_TS_ACTIVE_NODE = waitUntilNode(
      []{
        return rpm_signal.get_value() > Parameters::getInstance().rpmTsActiveThreshold;
      }, 
